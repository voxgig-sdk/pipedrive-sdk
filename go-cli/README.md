...
boru-driven command-line client **and** interactive REPL for the Pipedrive
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pipedrive-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pipedrive-cli
   ```
...
Run with no arguments to open a REPL (prompt `pipedrive>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/pipedrive-cli`. |
...
(`./pipedrive-cli <expr>`) and interactively (the REPL), and expressions compose the
