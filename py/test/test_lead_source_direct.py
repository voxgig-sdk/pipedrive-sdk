...
class TestLeadSourceDirect:

    def test_should_direct_list_lead_source(self):
        setup = _lead_source_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-lead_source", "live" if setup["live"] else "unit")
        if _skip:
...
def _lead_source_direct_setup(mockres):
