...
class PipedriveTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PipedriveTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PipedriveTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PipedriveTestRunner.getenv("PIPEDRIVE_TEST_LIVE")
        override = PipedriveTestRunner.getenv("PIPEDRIVE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PipedriveTestRunner.getenv(key)
        ...
        explain = PipedriveTestRunner.getenv("PIPEDRIVE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PIPEDRIVE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PipedriveTestRunner._test_control is not None:
            return PipedriveTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PipedriveTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PipedriveTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PipedriveTestRunner.load_test_control()
...
def load_env_local():
    PipedriveTestRunner.load_env_local()


def env_override(m):
    return PipedriveTestRunner.env_override(m)


def entity_data(v):
    return PipedriveTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PipedriveTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PipedriveTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PipedriveTestRunner.load_test_control()


def live_client_options():
    return PipedriveTestRunner.live_client_options()


def live_delay_ms():
    return PipedriveTestRunner.live_delay_ms()
