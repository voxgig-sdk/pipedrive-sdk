...
class TestTaskEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Task(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Task(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PIPEDRIVE_TEST_TASK_ENTID JSON to run live")
        client = setup["client"]
        ...
        task_ref01_list_result = task_ref01_ent.list(task_ref01_match, None)
        assert isinstance(task_ref01_list_result, list)
        ...
        task_ref01_list_rt0_result = task_ref01_ent.list(task_ref01_match_rt0, None)
        assert isinstance(task_ref01_list_rt0_result, list)
...
def _task_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("PIPEDRIVE_TEST_TASK_ENTID"))
