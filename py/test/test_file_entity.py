    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PIPEDRIVE_TEST_FILE_ENTID JSON to run live")
        client = setup["client"]
        ...
        file_ref01_markdef_up0_value = "Mark01-file_ref01_" + str(setup["now"])
        file_ref01_data_up0_up[file_ref01_markdef_up0_name] = file_ref01_markdef_up0_value

        file_ref01_resdata_up0 = helpers.to_map(runner.entity_data(file_ref01_ent.update(file_ref01_data_up0_up, None)))
        assert file_ref01_resdata_up0 is not None
        assert file_ref01_resdata_up0["id"] == file_ref01_data_up0_up["id"]
        assert file_ref01_resdata_up0[file_ref01_markdef_up0_name] == file_ref01_markdef_up0_value

        # LOAD
        file_ref01_match_dt0 = {
            "id": file_ref01_data["id"],
        }
        file_ref01_data_dt0_loaded = file_ref01_ent.load(file_ref01_match_dt0, None)
        file_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(file_ref01_data_dt0_loaded))
        assert file_ref01_data_dt0_load_result is not None
        assert file_ref01_data_dt0_load_result["id"] == file_ref01_data["id"]

        # REMOVE
        file_ref01_match_rm0 = {
            "id": file_ref01_data["id"],
        }
        file_ref01_ent.remove(file_ref01_match_rm0, None)

        # LIST
        file_ref01_match_rt0 = {}

        file_ref01_list_rt0_result = file_ref01_ent.list(file_ref01_match_rt0, None)
        assert isinstance(file_ref01_list_rt0_result, list)
...
def _file_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("PIPEDRIVE_TEST_FILE_ENTID"))
