    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PIPEDRIVE_TEST_PERSON_ENTID JSON to run live")
        client = setup["client"]
        ...
        person_ref01_data = helpers.to_map(runner.entity_data(person_ref01_ent.create(person_ref01_data, None)))
        assert person_ref01_data is not None
        assert person_ref01_data["id"] is not None
        ...
        person_ref01_resdata_up0 = helpers.to_map(runner.entity_data(person_ref01_ent.update(person_ref01_data_up0_up, None)))
        assert person_ref01_resdata_up0 is not None
        assert person_ref01_resdata_up0["id"] == person_ref01_data_up0_up["id"]

        # REMOVE
        person_ref01_match_rm0 = {
            "id": person_ref01_data["id"],
        }
        person_ref01_ent.remove(person_ref01_match_rm0, None)

        # LIST
        person_ref01_match_rt0 = {}

        person_ref01_list_rt0_result = person_ref01_ent.list(person_ref01_match_rt0, None)
        assert isinstance(person_ref01_list_rt0_result, list)
...
def _person_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("PIPEDRIVE_TEST_PERSON_ENTID"))
