...
class TestActivityFieldDirect:

    def test_should_direct_list_activity_field(self):
        setup = _activity_field_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-activity_field", "live" if setup["live"] else "unit")
        if _skip:
...
def _activity_field_direct_setup(mockres):
