...
class TestRecentDirect:

    def test_should_direct_list_recent(self):
        setup = _recent_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-recent", "live" if setup["live"] else "unit")
        if _skip:
...
def _recent_direct_setup(mockres):
