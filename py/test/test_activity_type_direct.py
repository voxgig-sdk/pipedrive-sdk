...
class TestActivityTypeDirect:

    def test_should_direct_list_activity_type(self):
        setup = _activity_type_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-activity_type", "live" if setup["live"] else "unit")
        if _skip:
...
def _activity_type_direct_setup(mockres):
