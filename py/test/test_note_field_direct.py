...
class TestNoteFieldDirect:

    def test_should_direct_list_note_field(self):
        setup = _note_field_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-note_field", "live" if setup["live"] else "unit")
        if _skip:
...
def _note_field_direct_setup(mockres):
