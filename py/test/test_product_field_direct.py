...
class TestProductFieldDirect:

    def test_should_direct_list_product_field(self):
        setup = _product_field_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-product_field", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_product_field(self):
        setup = _product_field_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-product_field", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _product_field_direct_setup(mockres):
