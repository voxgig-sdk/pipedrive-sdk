...
class TestOauthDirect:

    def test_should_direct_load_oauth(self):
        setup = _oauth_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-oauth", "live" if setup["live"] else "unit")
        if _skip:
...
def _oauth_direct_setup(mockres):
