...
class TestUserSettingEntity:

    def test_should_create_instance(self):
        testsdk = PipedriveSDK.test(None, None)
        ent = testsdk.UserSetting(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _user_setting_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "user_setting." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PIPEDRIVE_TEST_USER_SETTING_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        user_setting_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.user_setting")))
        user_setting_ref01_data = None
        if len(user_setting_ref01_data_raw) > 0:
            user_setting_ref01_data = helpers.to_map(user_setting_ref01_data_raw[0][1])

        # LOAD
        user_setting_ref01_ent = client.UserSetting(None)
        user_setting_ref01_match_dt0 = {}
        user_setting_ref01_data_dt0_loaded = user_setting_ref01_ent.load(user_setting_ref01_match_dt0, None)
        assert user_setting_ref01_data_dt0_loaded is not None



def _user_setting_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/user_setting/UserSettingTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "PIPEDRIVE_TEST_USER_SETTING_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("PIPEDRIVE_TEST_USER_SETTING_ENTID"))
