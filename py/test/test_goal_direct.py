...
class TestGoalDirect:

    def test_should_direct_load_goal(self):
        setup = _goal_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-goal", "live" if setup["live"] else "unit")
        if _skip:
...
def _goal_direct_setup(mockres):
