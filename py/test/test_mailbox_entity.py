...
class TestMailboxEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Mailbox(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Mailbox(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PIPEDRIVE_TEST_MAILBOX_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        mailbox_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.mailbox")))
        mailbox_ref01_data = None
        if len(mailbox_ref01_data_raw) > 0:
        ...
        mailbox_ref01_list_result = mailbox_ref01_ent.list(mailbox_ref01_match, None)
        assert isinstance(mailbox_ref01_list_result, list)
        ...
        mailbox_ref01_markdef_up0_value = "Mark01-mailbox_ref01_" + str(setup["now"])
        mailbox_ref01_data_up0_up[mailbox_ref01_markdef_up0_name] = mailbox_ref01_markdef_up0_value

        mailbox_ref01_resdata_up0 = helpers.to_map(runner.entity_data(mailbox_ref01_ent.update(mailbox_ref01_data_up0_up, None)))
        assert mailbox_ref01_resdata_up0 is not None
        assert mailbox_ref01_resdata_up0["id"] == mailbox_ref01_data_up0_up["id"]
        assert mailbox_ref01_resdata_up0[mailbox_ref01_markdef_up0_name] == mailbox_ref01_markdef_up0_value

        # LOAD
        mailbox_ref01_match_dt0 = {
            "id": mailbox_ref01_data["id"],
        }
        mailbox_ref01_data_dt0_loaded = mailbox_ref01_ent.load(mailbox_ref01_match_dt0, None)
        mailbox_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(mailbox_ref01_data_dt0_loaded))
        assert mailbox_ref01_data_dt0_load_result is not None
        assert mailbox_ref01_data_dt0_load_result["id"] == mailbox_ref01_data["id"]
...
def _mailbox_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("PIPEDRIVE_TEST_MAILBOX_ENTID"))
