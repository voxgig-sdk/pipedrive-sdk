...
from pipedrive_sdk.pipedrive_types import (
    PermissionSet,
    PermissionSetLoadMatch,
    PermissionSetListMatch,
)
...
class PermissionSetEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PermissionSetEntity(self._client, opts)
    ...
    def data_get(self) -> PermissionSet:
    ...
    def match_get(self) -> PermissionSet:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PermissionSet:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PermissionSet().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PermissionSet]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PermissionSet().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
