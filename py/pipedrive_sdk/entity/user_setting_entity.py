...
class UserSettingEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return UserSettingEntity(self._client, opts)
    ...
    def match_get(self) -> UserSetting:
    ...
    def load(self, reqmatch=None, ctrl=None) -> UserSetting:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.UserSetting().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
