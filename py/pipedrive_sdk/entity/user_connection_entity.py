...
class UserConnectionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return UserConnectionEntity(self._client, opts)
    ...
    def data_get(self) -> UserConnection:
    ...
    def match_get(self) -> UserConnection:
    ...
    def load(self, reqmatch=None, ctrl=None) -> UserConnection:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.UserConnection().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
