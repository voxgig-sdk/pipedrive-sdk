...
class ActivityTypeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ActivityTypeEntity(self._client, opts)
    ...
    def data_get(self) -> ActivityType:
    ...
    def match_get(self) -> ActivityType:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ActivityType]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ActivityType().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: ActivityTypeCreateData, ctrl=None) -> ActivityType:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: ActivityTypeUpdateData, ctrl=None) -> ActivityType:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ActivityType:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ActivityType().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
