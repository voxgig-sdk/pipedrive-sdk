...
[MCP](https://modelcontextprotocol.io) server exposing the Pipedrive SDK as
two agent tools — `pipedrive_list` and `pipedrive_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pipedrive-mcp)
make build
...
claude mcp add --scope user pipedrive \
  -- /absolute/path/to/pipedrive-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./pipedrive-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pipedrive-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user pipedrive \
     -- "$PWD"/dist/*/pipedrive-mcp -transport stdio
   ```
...
| Tool | Args | Returns |
|------|------|---------|
| `pipedrive_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `pipedrive_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PIPEDRIVE_APIKEY` | API key sent with every request. |
| `PIPEDRIVE_BASE` | Optional override of the API base URL. |
...
```sh
./pipedrive-mcp -transport http -addr :18080 &
