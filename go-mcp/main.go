// Command Pipedrive-mcp is an MCP server exposing the Pipedrive
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Pipedrive_list  — list records of an entity
//	Pipedrive_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: PIPEDRIVE_APIKEY carries the API key and
	// PIPEDRIVE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PIPEDRIVE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PIPEDRIVE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPipedriveSDK(opts)
