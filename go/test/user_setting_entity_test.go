...
func TestUserSettingEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_USER_SETTING_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		userSettingRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.user_setting")))
		var userSettingRef01Data map[string]any
		if len(userSettingRef01DataRaw) > 0 {
			userSettingRef01Data = core.ToMapAny(userSettingRef01DataRaw[0][1])
		}
		// Discard guards against Go's unused-var check when the flow's steps
		// happen not to consume the bootstrap data (e.g. list-only flows).
		_ = userSettingRef01Data
		...
		if userSettingRef01DataDt0Loaded == nil {
...
func user_settingBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"user_setting01", "user_setting02", "user_setting03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("PIPEDRIVE_TEST_USER_SETTING_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PIPEDRIVE_TEST_USER_SETTING_ENTID"])
