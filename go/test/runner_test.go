...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("PIPEDRIVE_TEST_LIVE") == "TRUE" ||
		os.Getenv("PIPEDRIVE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("PIPEDRIVE_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.PipedriveSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.PipedriveError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.PipedriveSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.PipedriveError{Msg: msg, Code: code}
}
