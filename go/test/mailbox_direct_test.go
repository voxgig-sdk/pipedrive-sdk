...
func TestMailboxDirect(t *testing.T) {
	t.Run("direct-list-mailbox", func(t *testing.T) {
		setup := mailboxDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-mailbox", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-mailbox", func(t *testing.T) {
		setup := mailboxDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-mailbox", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type mailboxDirectSetupResult struct {
...
func mailboxDirectSetup(mockres any) *mailboxDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_MAILBOX_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_MAILBOX_ENTID"]; ok {
		...
		return &mailboxDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &mailboxDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
