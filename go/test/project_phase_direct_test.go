...
func TestProjectPhaseDirect(t *testing.T) {
	t.Run("direct-list-project_phase", func(t *testing.T) {
		setup := project_phaseDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-project_phase", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-project_phase", func(t *testing.T) {
		setup := project_phaseDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-project_phase", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type project_phaseDirectSetupResult struct {
...
func project_phaseDirectSetup(mockres any) *project_phaseDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_PROJECT_PHASE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_PROJECT_PHASE_ENTID"]; ok {
		...
		return &project_phaseDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &project_phaseDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
