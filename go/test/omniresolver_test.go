...
import (
	...
	omni "github.com/voxgig-sdk/pipedrive-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PipedriveError); is {
				out := map[string]any{
					"name":    "PipedriveError",
...
func omniCtx(arg any, client *sdk.PipedriveSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PipedriveSDK); is {
			client = live
