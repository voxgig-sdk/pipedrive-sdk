...
func TestChannelEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_CHANNEL_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func channelBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"channel01", "channel02", "channel03", "conversation01", "conversation02", "conversation03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("PIPEDRIVE_TEST_CHANNEL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PIPEDRIVE_TEST_CHANNEL_ENTID"])
