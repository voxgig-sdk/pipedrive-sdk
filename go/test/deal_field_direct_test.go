...
func TestDealFieldDirect(t *testing.T) {
	t.Run("direct-list-deal_field", func(t *testing.T) {
		setup := deal_fieldDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-deal_field", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-deal_field", func(t *testing.T) {
		setup := deal_fieldDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-deal_field", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type deal_fieldDirectSetupResult struct {
...
func deal_fieldDirectSetup(mockres any) *deal_fieldDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_DEAL_FIELD_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_DEAL_FIELD_ENTID"]; ok {
		...
		return &deal_fieldDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &deal_fieldDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
