...
func TestLegacyTeamDirect(t *testing.T) {
	t.Run("direct-list-legacy_team", func(t *testing.T) {
		setup := legacy_teamDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-legacy_team", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-legacy_team", func(t *testing.T) {
		setup := legacy_teamDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-legacy_team", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type legacy_teamDirectSetupResult struct {
...
func legacy_teamDirectSetup(mockres any) *legacy_teamDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_LEGACY_TEAM_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_LEGACY_TEAM_ENTID"]; ok {
		...
		return &legacy_teamDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &legacy_teamDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
