...
func TestProjectBoardDirect(t *testing.T) {
	t.Run("direct-list-project_board", func(t *testing.T) {
		setup := project_boardDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-project_board", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-project_board", func(t *testing.T) {
		setup := project_boardDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-project_board", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type project_boardDirectSetupResult struct {
...
func project_boardDirectSetup(mockres any) *project_boardDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_PROJECT_BOARD_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_PROJECT_BOARD_ENTID"]; ok {
		...
		return &project_boardDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &project_boardDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
