...
func TestRoleDirect(t *testing.T) {
	t.Run("direct-list-role", func(t *testing.T) {
		setup := roleDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-role", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"role01"} {
	...
	t.Run("direct-load-role", func(t *testing.T) {
		setup := roleDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-role", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["id"] = setup.idmap["role01"]
...
type roleDirectSetupResult struct {
...
func roleDirectSetup(mockres any) *roleDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_ROLE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_ROLE_ENTID"]; ok {
		...
		return &roleDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &roleDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
