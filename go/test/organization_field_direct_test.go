...
func TestOrganizationFieldDirect(t *testing.T) {
	t.Run("direct-list-organization_field", func(t *testing.T) {
		setup := organization_fieldDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-organization_field", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-organization_field", func(t *testing.T) {
		setup := organization_fieldDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-organization_field", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type organization_fieldDirectSetupResult struct {
...
func organization_fieldDirectSetup(mockres any) *organization_fieldDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_ORGANIZATION_FIELD_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_ORGANIZATION_FIELD_ENTID"]; ok {
		...
		return &organization_fieldDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &organization_fieldDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
