...
func TestDealDirect(t *testing.T) {
	...
	t.Run("direct-load-deal", func(t *testing.T) {
		setup := dealDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-deal", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func dealDirectSetup(mockres any) *dealDirectSetupResult {
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_DEAL_ENTID"]; ok {
