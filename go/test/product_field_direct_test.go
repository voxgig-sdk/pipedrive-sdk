...
func TestProductFieldDirect(t *testing.T) {
	t.Run("direct-list-product_field", func(t *testing.T) {
		setup := product_fieldDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-product_field", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-product_field", func(t *testing.T) {
		setup := product_fieldDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-product_field", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type product_fieldDirectSetupResult struct {
...
func product_fieldDirectSetup(mockres any) *product_fieldDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_PRODUCT_FIELD_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_PRODUCT_FIELD_ENTID"]; ok {
		...
		return &product_fieldDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &product_fieldDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
