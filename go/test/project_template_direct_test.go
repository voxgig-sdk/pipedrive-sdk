...
func TestProjectTemplateDirect(t *testing.T) {
	t.Run("direct-list-project_template", func(t *testing.T) {
		setup := project_templateDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-project_template", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-project_template", func(t *testing.T) {
		setup := project_templateDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-project_template", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type project_templateDirectSetupResult struct {
...
func project_templateDirectSetup(mockres any) *project_templateDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_PROJECT_TEMPLATE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_PROJECT_TEMPLATE_ENTID"]; ok {
		...
		return &project_templateDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &project_templateDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
