...
func TestPermissionSetDirect(t *testing.T) {
	t.Run("direct-list-permission_set", func(t *testing.T) {
		setup := permission_setDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-permission_set", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"permission_set01"} {
		...
		if setup.live {
			params["id"] = setup.idmap["permission_set01"]
		} else {
			params["id"] = "direct01"
		}
	...
	t.Run("direct-load-permission_set", func(t *testing.T) {
		setup := permission_setDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-permission_set", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["id"] = setup.idmap["permission_set01"]
...
type permission_setDirectSetupResult struct {
...
func permission_setDirectSetup(mockres any) *permission_setDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_PERMISSION_SET_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_PERMISSION_SET_ENTID"]; ok {
		...
		return &permission_setDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &permission_setDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
