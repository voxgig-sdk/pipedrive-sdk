...
func TestPipelineDirect(t *testing.T) {
	t.Run("direct-list-pipeline", func(t *testing.T) {
		setup := pipelineDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-pipeline", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"pipeline01"} {
		...
		if setup.live {
			params["id"] = setup.idmap["pipeline01"]
		} else {
			params["id"] = "direct01"
		}
	...
	t.Run("direct-load-pipeline", func(t *testing.T) {
		setup := pipelineDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-pipeline", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["id"] = setup.idmap["pipeline01"]
...
type pipelineDirectSetupResult struct {
...
func pipelineDirectSetup(mockres any) *pipelineDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_PIPELINE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_PIPELINE_ENTID"]; ok {
		...
		return &pipelineDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &pipelineDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
