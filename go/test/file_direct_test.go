...
func TestFileDirect(t *testing.T) {
	t.Run("direct-list-file", func(t *testing.T) {
		setup := fileDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-file", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-file", func(t *testing.T) {
		setup := fileDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-file", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type fileDirectSetupResult struct {
...
func fileDirectSetup(mockres any) *fileDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_FILE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_FILE_ENTID"]; ok {
		...
		return &fileDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &fileDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
