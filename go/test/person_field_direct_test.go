...
func TestPersonFieldDirect(t *testing.T) {
	t.Run("direct-list-person_field", func(t *testing.T) {
		setup := person_fieldDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-person_field", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-person_field", func(t *testing.T) {
		setup := person_fieldDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-person_field", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type person_fieldDirectSetupResult struct {
...
func person_fieldDirectSetup(mockres any) *person_fieldDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_PERSON_FIELD_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_PERSON_FIELD_ENTID"]; ok {
		...
		return &person_fieldDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &person_fieldDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
