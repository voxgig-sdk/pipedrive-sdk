...
func TestNoteDirect(t *testing.T) {
	t.Run("direct-list-note", func(t *testing.T) {
		setup := noteDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-note", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-note", func(t *testing.T) {
		setup := noteDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-note", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			params["comment_id"] = setup.idmap["comment01"]
		} else {
			params["comment_id"] = "direct01"
			params["id"] = "direct02"
		}

		result, err := client.Direct(map[string]any{
			"path":   "notes/{id}/comments/{comment_id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type noteDirectSetupResult struct {
...
func noteDirectSetup(mockres any) *noteDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_NOTE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_NOTE_ENTID"]; ok {
		...
		return &noteDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &noteDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
