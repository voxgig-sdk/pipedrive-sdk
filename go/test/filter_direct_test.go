...
func TestFilterDirect(t *testing.T) {
	...
	t.Run("direct-load-filter", func(t *testing.T) {
		setup := filterDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-filter", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func filterDirectSetup(mockres any) *filterDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_FILTER_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_FILTER_ENTID"]; ok {
