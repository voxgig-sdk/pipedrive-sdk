...
func TestTaskDirect(t *testing.T) {
	t.Run("direct-list-task", func(t *testing.T) {
		setup := taskDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-task", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-task", func(t *testing.T) {
		setup := taskDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-task", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type taskDirectSetupResult struct {
...
func taskDirectSetup(mockres any) *taskDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PIPEDRIVE_TEST_TASK_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PIPEDRIVE_TEST_TASK_ENTID"]; ok {
		...
		return &taskDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &taskDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
