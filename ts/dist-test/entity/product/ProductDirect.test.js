...
(0, node_test_1.describe)('ProductDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-product', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-product', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["product01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.id = setup.idmap['product01'];
        }
        else {
            params.id = 'direct01';
        }
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['PIPEDRIVE_TEST_PRODUCT_ENTID'];
