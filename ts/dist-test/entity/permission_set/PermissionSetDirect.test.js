...
(0, node_test_1.describe)('PermissionSetDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.PipedriveSDK({
    ...
    (0, node_test_1.test)('direct-load-permission_set', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-permission_set', setup.live))
            return;
        ...
        if (setup.live) {
            const listResult = await client.direct({
                ...
                method: 'GET',
                params: {
                    id: setup.idmap['permission_set01'],
                },
            });
    ...
    (0, node_test_1.test)('direct-list-permission_set', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-permission_set', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["permission_set01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.PIPEDRIVE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['PIPEDRIVE_TEST_PERMISSION_SET_ENTID'];
    ...
    const client = new __1.PipedriveSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=PermissionSetDirect.test.js.map