...
(0, node_test_1.describe)('StageEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list']) {
            if (!live && (0, utility_1.maybeSkipControl)(t, 'entityOp', 'stage.' + op, live))
                return;
        }
        ...
        let stage_ref01_data = Object.values(setup.data.existing.stage)[0];
        // LIST
        const stage_ref01_ent = client.Stage();
        const stage_ref01_match = {};
        const stage_ref01_list = (await stage_ref01_ent.list(stage_ref01_match)).map((e) => e.data());
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/stage/StageTestData.json');
    ...
    let idmap = transform(['stage01', 'stage02', 'stage03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
...
//# sourceMappingURL=StageEntity.test.js.map