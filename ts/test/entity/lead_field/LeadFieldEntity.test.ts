...
describe('LeadFieldEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'lead_field.' + op, live)) return
    }
    ...
    let lead_field_ref01_data = Object.values(setup.data.existing.lead_field)[0] as any

    // LIST
    const lead_field_ref01_ent = client.LeadField()
    const lead_field_ref01_match: any = {}

    const lead_field_ref01_list = (await lead_field_ref01_ent.list(lead_field_ref01_match)).map((e: any) => e.data())
