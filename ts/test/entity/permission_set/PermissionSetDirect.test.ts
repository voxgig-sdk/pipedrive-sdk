...
describe('PermissionSetDirect', async () => {
  ...
  test('direct-load-permission_set', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-permission_set', setup.live)) return
  ...
  test('direct-list-permission_set', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-permission_set', setup.live)) return
    if (skipIfMissingIds(t, setup, ["permission_set01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PIPEDRIVE_TEST_PERMISSION_SET_ENTID']
