...
describe('ProjectTemplateEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'project_template.' + op, live)) return
    }
    ...
    let project_template_ref01_data = Object.values(setup.data.existing.project_template)[0] as any

    // LIST
    const project_template_ref01_ent = client.ProjectTemplate()
    const project_template_ref01_match: any = {}

    const project_template_ref01_list = (await project_template_ref01_ent.list(project_template_ref01_match)).map((e: any) => e.data())


    // LOAD
    const project_template_ref01_match_dt0: any = {}
    ...
    assert(project_template_ref01_data_dt0.id === project_template_ref01_data.id)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/project_template/ProjectTemplateTestData.json')
