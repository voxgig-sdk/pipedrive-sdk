...
describe('ActivityFieldEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'activity_field.' + op, live)) return
    }
    ...
    let activity_field_ref01_data = Object.values(setup.data.existing.activity_field)[0] as any

    // LIST
    const activity_field_ref01_ent = client.ActivityField()
    const activity_field_ref01_match: any = {}

    const activity_field_ref01_list = (await activity_field_ref01_ent.list(activity_field_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/activity_field/ActivityFieldTestData.json')
  ...
  let idmap = transform(
    ['activity_field01','activity_field02','activity_field03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
