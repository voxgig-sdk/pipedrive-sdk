...
describe('NoteFieldEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'note_field.' + op, live)) return
    }
    ...
    let note_field_ref01_data = Object.values(setup.data.existing.note_field)[0] as any

    // LIST
    const note_field_ref01_ent = client.NoteField()
    const note_field_ref01_match: any = {}

    const note_field_ref01_list = (await note_field_ref01_ent.list(note_field_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/note_field/NoteFieldTestData.json')
  ...
  let idmap = transform(
    ['note_field01','note_field02','note_field03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
