...
describe('ProjectBoardEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'project_board.' + op, live)) return
    }
    ...
    let project_board_ref01_data = Object.values(setup.data.existing.project_board)[0] as any

    // LIST
    const project_board_ref01_ent = client.ProjectBoard()
    const project_board_ref01_match: any = {}

    const project_board_ref01_list = (await project_board_ref01_ent.list(project_board_ref01_match)).map((e: any) => e.data())


    // LOAD
    const project_board_ref01_match_dt0: any = {}
    ...
    assert(project_board_ref01_data_dt0.id === project_board_ref01_data.id)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/project_board/ProjectBoardTestData.json')
  ...
  let idmap = transform(
    ['project_board01','project_board02','project_board03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
