...
describe('UserSettingEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'user_setting.' + op, live)) return
    }
    ...
    let user_setting_ref01_data = Object.values(setup.data.existing.user_setting)[0] as any

    // LOAD
    const user_setting_ref01_ent = client.UserSetting()
    const user_setting_ref01_match_dt0: any = {}
    const user_setting_ref01_data_dt0 = (await user_setting_ref01_ent.load(user_setting_ref01_match_dt0)).data()
    assert(null != user_setting_ref01_data_dt0)
