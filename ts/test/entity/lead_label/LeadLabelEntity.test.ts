...
describe('LeadLabelEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update', 'remove']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'lead_label.' + op, live)) return
    }
    ...
    // LIST
    const lead_label_ref01_match: any = {}

    const lead_label_ref01_list = (await lead_label_ref01_ent.list(lead_label_ref01_match)).map((e: any) => e.data())

    assert(!isempty(select(lead_label_ref01_list, { id: lead_label_ref01_data.id })))


    // UPDATE
    const lead_label_ref01_data_up0: any = {}
    ...
    ;(lead_label_ref01_data_up0 as any)[lead_label_ref01_markdef_up0.name] = lead_label_ref01_markdef_up0.value
    ...
    assert((lead_label_ref01_resdata_up0 as any)[lead_label_ref01_markdef_up0.name] === lead_label_ref01_markdef_up0.value)


    // REMOVE
    const lead_label_ref01_match_rm0: any = { id: lead_label_ref01_data.id }
    await lead_label_ref01_ent.remove(lead_label_ref01_match_rm0)
  

    // LIST
    const lead_label_ref01_match_rt0: any = {}

    const lead_label_ref01_list_rt0 = (await lead_label_ref01_ent.list(lead_label_ref01_match_rt0)).map((e: any) => e.data())
