...
class TaskEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Task(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Task(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = task_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load", "remove"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "task." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_TASK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $task_ref01_list_result = $task_ref01_ent->list($task_ref01_match, null);
        $this->assertIsArray($task_ref01_list_result);
        ...
        $task_ref01_ent->remove($task_ref01_match_rm0, null);

        // LIST
        $task_ref01_match_rt0 = [];

        $task_ref01_list_rt0_result = $task_ref01_ent->list($task_ref01_match_rt0, null);
        $this->assertIsArray($task_ref01_list_rt0_result);
...
function task_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/task/TaskTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["task01", "task02", "task03"] as $k) {
    ...
    $entid_env_raw = getenv("PIPEDRIVE_TEST_TASK_ENTID");
