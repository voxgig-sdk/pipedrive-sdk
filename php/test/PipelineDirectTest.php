...
class PipelineDirectTest extends TestCase
{
    public function test_direct_list_pipeline(): void
    {
        $setup = pipeline_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-pipeline", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["pipeline01"] as $_liveKey) {
    ...
    public function test_direct_load_pipeline(): void
    {
        $setup = pipeline_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-pipeline", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
...
function pipeline_direct_setup($mockres)
{
