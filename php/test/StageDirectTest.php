...
class StageDirectTest extends TestCase
{
    public function test_direct_list_stage(): void
    {
        $setup = stage_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-stage", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["stage01"] as $_liveKey) {
...
function stage_direct_setup($mockres)
{
