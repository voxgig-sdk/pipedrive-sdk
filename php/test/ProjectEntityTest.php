...
class ProjectEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_PROJECT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function project_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["project01", "project02", "project03", "activity01", "activity02", "activity03", "task01", "task02", "task03"] as $k) {
    ...
    $entid_env_raw = getenv("PIPEDRIVE_TEST_PROJECT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["PIPEDRIVE_TEST_PROJECT_ENTID"]);
