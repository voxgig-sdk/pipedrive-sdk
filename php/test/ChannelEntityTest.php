...
class ChannelEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_CHANNEL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function channel_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["channel01", "channel02", "channel03", "conversation01", "conversation02", "conversation03"] as $k) {
    ...
    $entid_env_raw = getenv("PIPEDRIVE_TEST_CHANNEL_ENTID");
