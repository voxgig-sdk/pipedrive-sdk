...
class UserSettingEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_USER_SETTING_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $this->assertNotNull($user_setting_ref01_data_dt0_loaded);
...
function user_setting_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("PIPEDRIVE_TEST_USER_SETTING_ENTID");
