...
// Pipedrive SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = PipedriveOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(PipedriveOmni::structfix($now, Struct::undef()))
                        !== json_encode(PipedriveOmni::structfix($val, Struct::undef()))) {
