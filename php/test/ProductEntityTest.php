...
class ProductEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_PRODUCT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function product_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["product01", "product02", "product03", "follower01", "follower02", "follower03"] as $k) {
    ...
    $entid_env_raw = getenv("PIPEDRIVE_TEST_PRODUCT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["PIPEDRIVE_TEST_PRODUCT_ENTID"]);
