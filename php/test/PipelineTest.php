...
{
    private static function utility(): PipedriveUtility
    {
        return new PipedriveUtility();
    }
    ...
    private static function ctx(array $over = []): PipedriveContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PipedriveContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PipedriveOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PipedriveError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PipedriveOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PipedriveMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PipedriveOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PipedriveMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PipedriveOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PipedriveMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PipedriveSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PipedriveMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PipedriveResponse([]),
            'result' => new PipedriveResult([]),
        ]);
        [, $err] = PipedriveMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PipedriveSpec([]),
            'response' => null,
            'result' => new PipedriveResult([]),
        ]);
        [, $err] = PipedriveMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PipedriveSpec(['step' => 's']),
            'response' => new PipedriveResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PipedriveResult([]),
        ]);
        [, $err] = PipedriveMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PipedriveSpec(['step' => 's']),
            'response' => new PipedriveResponse(self::resp(200, ['v' => 1])),
            'result' => new PipedriveResult([]),
        ]);
        [, $err] = PipedriveMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PipedriveSpec(['step' => 's']),
            'response' => new PipedriveResponse(self::resp(200, ['v' => 2])),
            'result' => new PipedriveResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PipedriveMakeResponse::call($ctx);
    ...
    {
        $preset = new PipedriveResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PipedriveSpec([]),
            'response' => new PipedriveResponse([]),
            'result' => new PipedriveResult([]),
        ...
        [$got, $err] = PipedriveMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PipedriveResult([])]);
        [, $err] = PipedriveMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PipedriveSpec([]), 'result' => null]);
        [, $err] = PipedriveMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PipedriveSpec(['step' => 's']),
            'result' => new PipedriveResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PipedriveOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PipedriveMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PipedriveSpec(['step' => 's']),
            'result' => new PipedriveResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PipedriveOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PipedriveMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PipedriveResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PipedriveSpec([]),
            'result' => new PipedriveResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PipedriveMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PipedriveMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PipedriveError('boom', 'boom');
        $utility->fetcher = function (PipedriveContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PipedriveSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PipedriveMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PipedriveContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PipedriveMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PipedriveResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PipedriveContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PipedriveSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PipedriveMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PipedriveResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PipedriveSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PipedriveMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PipedriveMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PipedriveMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PipedriveResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PipedriveDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PipedriveResult(['ok' => false])]);
        $this->expectException(PipedriveError::class);
        PipedriveDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PipedriveResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PipedriveMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PipedriveMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PipedriveBaseFeature();
        $b = new PipedriveBaseFeature();
        PipedriveFeatureAdd::call($ctx, $a);
        PipedriveFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PipedriveBaseFeature
    {
        $f = new PipedriveBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PipedriveFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): PipedriveContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PipedriveSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PipedrivePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PipedriveResponse(['headers' => null]),
            'result' => new PipedriveResult([]),
        ]);
        PipedriveResultHeaders::call($ctx);
