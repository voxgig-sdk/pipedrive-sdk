...
class FtCtrl extends PipedriveControl
{
...
class FtClient
{
    ...
    private PipedriveUtility $utility;

    public function __construct(PipedriveUtility $utility, array $options)
    {
    ...
    public function get_utility(): PipedriveUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PipedriveContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PipedriveContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PipedriveContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PipedriveConfig::make_feature($name);
    ...
    private static function buildUrl(PipedriveSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PipedriveError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PipedriveResponse($fetched);
        }
    ...
    private function fail(PipedriveContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PipedriveContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
