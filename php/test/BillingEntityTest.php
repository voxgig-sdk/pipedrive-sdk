...
class BillingEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_BILLING_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function billing_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("PIPEDRIVE_TEST_BILLING_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["PIPEDRIVE_TEST_BILLING_ENTID"]);
