...
The PHP SDK for the Pipedrive API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->ActivityField()` — with named operations (`list`/`load`/`create`/`update`/`remove`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
try {
    ...
    foreach ($activityfields as $record) {
        $item = $record->data_get();
        echo $item["additional_data"] . "\n";
    }
} catch (\Throwable $err) {
...
$client = PipedriveSDK::test([
    "entity" => ["dealfield" => ["test01" => ["id" => "test01"]]],
]);

// list() returns entity instances (throws on error);
// call data_get() for the mock record.
$dealfield = $client->DealField()->list();
print_r(array_map(fn($item) => $item->data_get(), $dealfield));
```
...
| --- | --- | --- |
| `additional_data` | `array` | The additional data of the list |
| `data` | `array` |  |
...
```php
// list() returns an array of ActivityField records (throws on error).
...
| `data` | `array` | The array of activity types |
...
```php
// list() returns an array of ActivityType records (throws on error).
...
| Field | Type | Description |
| --- | --- | --- |
| `data` | `array` | An array of add-ons that the company has. |
| `success` | `bool` | If the response is successful or not |
...
```php
// load() returns the ENTITY — call data_get() for the CallLog record (throws on error).
...
```php
// list() returns an array of CallLog records (throws on error).
...
| `totals` | `array` | The total values of deals for the given period |
| `values_total` | `array` | The total values of the deals grouped by deal currency |
| `weighted_values_total` | `array` | The total weighted values of the deals grouped by deal currency. |
...
| `options` | `array` | When `field_type` is either set or enum, possible options must be supplied as a JSON-encoded sequential array of objects. |
| `success` | `bool` | If the response is successful or not |

#### Example: Load

```php
// load() returns the ENTITY — call data_get() for the DealField record (throws on error).
...
```php
// list() returns an array of DealField records (throws on error).
...
| Field | Type | Description |
| --- | --- | --- |
| `conditions` | `array` | The conditions of the filter as a JSON object. |
| `data` | `array` | The filter object including conditions |
| `id` | `string` |  |
...
```php
// load() returns the ENTITY — call data_get() for the Filter record (throws on error).
...
| Field | Type | Description |
| --- | --- | --- |
| `assignee` | `array` | Who this goal is assigned to. |
| `duration` | `array` | The date when the goal starts and ends. |
| `expected_outcome` | `array` | The expected outcome of the goal. |
| `goal` | `array` |  |
| `id` | `string` |  |
| `interval` | `string` | The interval of the goal |
| `title` | `string` | The title of the goal |
| `type` | `array` | The type of the goal. |

#### Example: Load

```php
// load() returns the ENTITY — call data_get() for the Goal record (throws on error).
...
| `label_ids` | `array` | The IDs of the lead labels which are associated with the lead |
...
| `value` | `array` | The potential value of the lead represented by a JSON object: `{ "amount": 200, "currency": "EUR" }`. |
...
```php
// load() returns the ENTITY — call data_get() for the Lead record (throws on error).
...
```php
// list() returns an array of Lead records (throws on error).
...
```php
// list() returns an array of LeadField records (throws on error).
...
```php
// list() returns an array of LeadLabel records (throws on error).
...
```php
// list() returns an array of LeadSource records (throws on error).
...
```php
// load() returns the ENTITY — call data_get() for the LegacyTeam record (throws on error).
...
```php
// list() returns an array of LegacyTeam records (throws on error).
...
| Field | Type | Description |
| --- | --- | --- |
| `data` | `array` | The mail thread object |
| `id` | `string` |  |
...
```php
// load() returns the ENTITY — call data_get() for the Mailbox record (throws on error).
...
```php
// list() returns an array of Mailbox records (throws on error).
$mailboxs = $client->Mailbox()->list();
```
...
| `organization` | `array` | The organization the note is attached to |
| `person` | `array` | The person the note is attached to |
...
| `project` | `array` | The project the note is attached to |
| `project_id` | `int` | The ID of the project the note is attached to |
| `task` | `array` | The task the note is attached to |
...
| `user` | `array` | The user who created the note |
...
```php
// list() returns an array of NoteField records (throws on error).
...
```php
// load() returns the ENTITY — call data_get() for the OrganizationField record (throws on error).
...
```php
// list() returns an array of OrganizationField records (throws on error).
...
| --- | --- | --- |
| `additional_data` | `array` | The additional data of the list |
| `data` | `mixed` | The array of organization relationships |
| `id` | `string` |  |
...
```php
// load() returns the ENTITY — call data_get() for the OrganizationRelationship record (throws on error).
...
```php
// list() returns an array of OrganizationRelationship records (throws on error).
$organization_relationships = $client->OrganizationRelationship()->list();
```
...
| `contents` | `array` | A permission assigned to this permission set |
| `data` | `array` | The array of permission set |
...
```php
// load() returns the ENTITY — call data_get() for the PermissionSet record (throws on error).
...
```php
// list() returns an array of PermissionSet records (throws on error).
...
```php
// load() returns the ENTITY — call data_get() for the PersonField record (throws on error).
...
```php
// list() returns an array of PersonField records (throws on error).
...
```php
// load() returns the ENTITY — call data_get() for the Pipeline record (throws on error).
...
```php
// list() returns an array of Pipeline records (throws on error).
...
```php
// load() returns the ENTITY — call data_get() for the ProductField record (throws on error).
...
```php
// list() returns an array of ProductField records (throws on error).
...
```php
// load() returns the ENTITY — call data_get() for the ProjectBoard record (throws on error).
...
```php
// list() returns an array of ProjectBoard records (throws on error).
...
```php
// load() returns the ENTITY — call data_get() for the ProjectPhase record (throws on error).
...
```php
// list() returns an array of ProjectPhase records (throws on error).
$project_phases = $client->ProjectPhase()->list();
```
...
```php
// load() returns the ENTITY — call data_get() for the ProjectTemplate record (throws on error).
...
```php
// list() returns an array of ProjectTemplate records (throws on error).
...
```php
// list() returns an array of Recent records (throws on error).
$recents = $client->Recent()->list();
```
...
| Field | Type | Description |
| --- | --- | --- |
| `additional_data` | `array` | The additional data in the role |
| `data` | `mixed` | The details of the sub-role |
| `id` | `string` |  |
...
```php
// list() returns an array of Stage records (throws on error).
...
```php
// list() returns an array of Task records (throws on error).
...
| Field | Type | Description |
| --- | --- | --- |
| `access` | `array` | The access given to the user. |
| `active_flag` | `bool` | Whether the user is active or not. |
| `data` | `array` |  |
...
| Field | Type | Description |
| --- | --- | --- |
| `data` | `array` | The object of UserConnections |
| `success` | `bool` | If the response is successful or not |

#### Example: Load

```php
// load() returns the ENTITY — call data_get() for the UserConnection record (throws on error).
...
| Field | Type | Description |
| --- | --- | --- |
| `data` | `array` | The array of Webhooks |
...
```
php/
├── pipedrive_sdk.php          -- Main SDK class
...
The main class (`pipedrive_sdk.php`) exports the SDK class
