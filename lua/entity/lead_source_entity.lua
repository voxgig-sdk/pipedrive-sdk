...
local LeadSourceEntity = {}
LeadSourceEntity.__index = LeadSourceEntity


function LeadSourceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LeadSourceEntity)
  self._name = "lead_source"
  self._client = client
...
function LeadSourceEntity:get_name()
  return self._name
end


function LeadSourceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LeadSourceEntity.new(self._client, opts)
end
...
function LeadSourceEntity:mark_deleted()
  self._deleted = true
end


function LeadSourceEntity:deleted()
  return true == self._deleted
end


function LeadSourceEntity:data_set(args)
  if args ~= nil then
...
function LeadSourceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LeadSourceEntity:match_set(args)
  if args ~= nil then
...
function LeadSourceEntity:match_get()
...
function LeadSourceEntity:stream(action, args, callopts)
...
---@param reqmatch LeadSourceListMatch
---@param ctrl? table
---@return LeadSource[]
---@return string? err
function LeadSourceEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function LeadSourceEntity:_run_op(ctx, post_done)
  local utility = self._utility
