...
local GoalEntity = {}
GoalEntity.__index = GoalEntity


function GoalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GoalEntity)
  self._name = "goal"
  self._client = client
...
function GoalEntity:get_name()
  return self._name
end


function GoalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GoalEntity.new(self._client, opts)
end
...
function GoalEntity:mark_deleted()
  self._deleted = true
end


function GoalEntity:deleted()
  return true == self._deleted
end


function GoalEntity:data_set(args)
  if args ~= nil then
...
function GoalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GoalEntity:match_set(args)
  if args ~= nil then
...
function GoalEntity:stream(action, args, callopts)
...
---@param reqmatch GoalLoadMatch
---@param ctrl? table
---@return Goal
---@return string? err
function GoalEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata GoalCreateData
---@param ctrl? table
---@return Goal
---@return string? err
function GoalEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata GoalUpdateData
---@param ctrl? table
---@return Goal
---@return string? err
function GoalEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch GoalRemoveMatch
---@param ctrl? table
---@return Goal
---@return string? err
function GoalEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function GoalEntity:_run_op(ctx, post_done)
  local utility = self._utility
