...
local LeadFieldEntity = {}
LeadFieldEntity.__index = LeadFieldEntity


function LeadFieldEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LeadFieldEntity)
  self._name = "lead_field"
  self._client = client
...
function LeadFieldEntity:get_name()
  return self._name
end


function LeadFieldEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LeadFieldEntity.new(self._client, opts)
end
...
function LeadFieldEntity:mark_deleted()
  self._deleted = true
end


function LeadFieldEntity:deleted()
  return true == self._deleted
end


function LeadFieldEntity:data_set(args)
  if args ~= nil then
...
function LeadFieldEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LeadFieldEntity:match_set(args)
  if args ~= nil then
...
function LeadFieldEntity:match_get()
...
function LeadFieldEntity:stream(action, args, callopts)
...
---@param reqmatch LeadFieldListMatch
---@param ctrl? table
---@return LeadField[]
---@return string? err
function LeadFieldEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function LeadFieldEntity:_run_op(ctx, post_done)
  local utility = self._utility
