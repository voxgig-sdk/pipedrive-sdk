...
local NoteFieldEntity = {}
NoteFieldEntity.__index = NoteFieldEntity


function NoteFieldEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NoteFieldEntity)
  self._name = "note_field"
  self._client = client
...
function NoteFieldEntity:get_name()
  return self._name
end


function NoteFieldEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NoteFieldEntity.new(self._client, opts)
end
...
function NoteFieldEntity:mark_deleted()
  self._deleted = true
end


function NoteFieldEntity:deleted()
  return true == self._deleted
end


function NoteFieldEntity:data_set(args)
  if args ~= nil then
...
function NoteFieldEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NoteFieldEntity:match_set(args)
  if args ~= nil then
...
function NoteFieldEntity:match_get()
...
function NoteFieldEntity:stream(action, args, callopts)
...
---@param reqmatch NoteFieldListMatch
---@param ctrl? table
---@return NoteField[]
---@return string? err
function NoteFieldEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function NoteFieldEntity:_run_op(ctx, post_done)
  local utility = self._utility
