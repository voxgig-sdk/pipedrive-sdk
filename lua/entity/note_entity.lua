...
local NoteEntity = {}
NoteEntity.__index = NoteEntity


function NoteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NoteEntity)
  self._name = "note"
  self._client = client
...
function NoteEntity:get_name()
  return self._name
end


function NoteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NoteEntity.new(self._client, opts)
end
...
function NoteEntity:mark_deleted()
  self._deleted = true
end


function NoteEntity:deleted()
  return true == self._deleted
end


function NoteEntity:data_set(args)
  if args ~= nil then
...
function NoteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NoteEntity:match_set(args)
  if args ~= nil then
...
function NoteEntity:stream(action, args, callopts)
...
---@param reqmatch NoteLoadMatch
---@param ctrl? table
---@return Note
---@return string? err
function NoteEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch NoteListMatch
---@param ctrl? table
---@return Note[]
---@return string? err
function NoteEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata NoteCreateData
---@param ctrl? table
---@return Note
---@return string? err
function NoteEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata NoteUpdateData
---@param ctrl? table
---@return Note
---@return string? err
function NoteEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch NoteRemoveMatch
---@param ctrl? table
---@return Note
---@return string? err
function NoteEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function NoteEntity:_run_op(ctx, post_done)
  local utility = self._utility
