...
local CallLogEntity = {}
CallLogEntity.__index = CallLogEntity


function CallLogEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CallLogEntity)
  self._name = "call_log"
  self._client = client
...
function CallLogEntity:get_name()
  return self._name
end


function CallLogEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CallLogEntity.new(self._client, opts)
end
...
function CallLogEntity:mark_deleted()
  self._deleted = true
end


function CallLogEntity:deleted()
  return true == self._deleted
end


function CallLogEntity:data_set(args)
  if args ~= nil then
...
function CallLogEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CallLogEntity:match_set(args)
  if args ~= nil then
...
function CallLogEntity:match_get()
...
function CallLogEntity:stream(action, args, callopts)
...
---@param reqmatch CallLogLoadMatch
---@param ctrl? table
---@return CallLog
---@return string? err
function CallLogEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CallLogListMatch
---@param ctrl? table
---@return CallLog[]
---@return string? err
function CallLogEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata CallLogCreateData
---@param ctrl? table
---@return CallLog
---@return string? err
function CallLogEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch CallLogRemoveMatch
---@param ctrl? table
---@return CallLog
---@return string? err
function CallLogEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function CallLogEntity:_run_op(ctx, post_done)
  local utility = self._utility
