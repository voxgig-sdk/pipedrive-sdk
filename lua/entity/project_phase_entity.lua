...
local ProjectPhaseEntity = {}
ProjectPhaseEntity.__index = ProjectPhaseEntity


function ProjectPhaseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectPhaseEntity)
  self._name = "project_phase"
  self._client = client
...
function ProjectPhaseEntity:get_name()
  return self._name
end


function ProjectPhaseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectPhaseEntity.new(self._client, opts)
end
...
function ProjectPhaseEntity:mark_deleted()
  self._deleted = true
end


function ProjectPhaseEntity:deleted()
  return true == self._deleted
end


function ProjectPhaseEntity:data_set(args)
  if args ~= nil then
...
function ProjectPhaseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectPhaseEntity:match_set(args)
  if args ~= nil then
...
function ProjectPhaseEntity:match_get()
...
function ProjectPhaseEntity:stream(action, args, callopts)
...
---@param reqmatch ProjectPhaseLoadMatch
---@param ctrl? table
---@return ProjectPhase
---@return string? err
function ProjectPhaseEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ProjectPhaseListMatch
---@param ctrl? table
---@return ProjectPhase[]
---@return string? err
function ProjectPhaseEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ProjectPhaseEntity:_run_op(ctx, post_done)
  local utility = self._utility
