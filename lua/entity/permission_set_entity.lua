...
local PermissionSetEntity = {}
PermissionSetEntity.__index = PermissionSetEntity


function PermissionSetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PermissionSetEntity)
  self._name = "permission_set"
  self._client = client
...
function PermissionSetEntity:get_name()
  return self._name
end


function PermissionSetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PermissionSetEntity.new(self._client, opts)
end
...
function PermissionSetEntity:mark_deleted()
  self._deleted = true
end


function PermissionSetEntity:deleted()
  return true == self._deleted
end


function PermissionSetEntity:data_set(args)
  if args ~= nil then
...
function PermissionSetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PermissionSetEntity:match_set(args)
  if args ~= nil then
...
function PermissionSetEntity:match_get()
...
function PermissionSetEntity:stream(action, args, callopts)
...
---@param reqmatch PermissionSetLoadMatch
---@param ctrl? table
---@return PermissionSet
---@return string? err
function PermissionSetEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PermissionSetListMatch
---@param ctrl? table
---@return PermissionSet[]
---@return string? err
function PermissionSetEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function PermissionSetEntity:_run_op(ctx, post_done)
  local utility = self._utility
