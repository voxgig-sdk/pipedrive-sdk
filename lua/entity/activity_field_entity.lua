...
local ActivityFieldEntity = {}
ActivityFieldEntity.__index = ActivityFieldEntity


function ActivityFieldEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActivityFieldEntity)
  self._name = "activity_field"
  self._client = client
...
function ActivityFieldEntity:get_name()
  return self._name
end


function ActivityFieldEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActivityFieldEntity.new(self._client, opts)
end
...
function ActivityFieldEntity:mark_deleted()
  self._deleted = true
end


function ActivityFieldEntity:deleted()
  return true == self._deleted
end


function ActivityFieldEntity:data_set(args)
  if args ~= nil then
...
function ActivityFieldEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActivityFieldEntity:match_set(args)
  if args ~= nil then
...
function ActivityFieldEntity:match_get()
...
function ActivityFieldEntity:stream(action, args, callopts)
...
---@param reqmatch ActivityFieldListMatch
---@param ctrl? table
---@return ActivityField[]
---@return string? err
function ActivityFieldEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ActivityFieldEntity:_run_op(ctx, post_done)
  local utility = self._utility
