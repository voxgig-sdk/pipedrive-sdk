...
local ProjectBoardEntity = {}
ProjectBoardEntity.__index = ProjectBoardEntity


function ProjectBoardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectBoardEntity)
  self._name = "project_board"
  self._client = client
...
function ProjectBoardEntity:get_name()
  return self._name
end


function ProjectBoardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectBoardEntity.new(self._client, opts)
end
...
function ProjectBoardEntity:mark_deleted()
  self._deleted = true
end


function ProjectBoardEntity:deleted()
  return true == self._deleted
end


function ProjectBoardEntity:data_set(args)
  if args ~= nil then
...
function ProjectBoardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectBoardEntity:match_set(args)
  if args ~= nil then
...
function ProjectBoardEntity:match_get()
...
function ProjectBoardEntity:stream(action, args, callopts)
...
---@param reqmatch ProjectBoardLoadMatch
---@param ctrl? table
---@return ProjectBoard
---@return string? err
function ProjectBoardEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ProjectBoardListMatch
---@param ctrl? table
---@return ProjectBoard[]
---@return string? err
function ProjectBoardEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ProjectBoardEntity:_run_op(ctx, post_done)
  local utility = self._utility
