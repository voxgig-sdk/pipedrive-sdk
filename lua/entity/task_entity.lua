...
function TaskEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TaskEntity)
  self._name = "task"
  self._client = client
...
function TaskEntity:get_name()
  return self._name
end


function TaskEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TaskEntity.new(self._client, opts)
end
...
function TaskEntity:mark_deleted()
  self._deleted = true
end


function TaskEntity:deleted()
  return true == self._deleted
end


function TaskEntity:data_set(args)
  if args ~= nil then
...
function TaskEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TaskEntity:match_set(args)
  if args ~= nil then
...
function TaskEntity:stream(action, args, callopts)
...
---@param reqmatch TaskLoadMatch
---@param ctrl? table
---@return Task
---@return string? err
function TaskEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch TaskListMatch
---@param ctrl? table
---@return Task[]
---@return string? err
function TaskEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata TaskCreateData
---@param ctrl? table
---@return Task
---@return string? err
function TaskEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata TaskUpdateData
---@param ctrl? table
---@return Task
---@return string? err
function TaskEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch TaskRemoveMatch
---@param ctrl? table
---@return Task
---@return string? err
function TaskEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function TaskEntity:_run_op(ctx, post_done)
  local utility = self._utility
