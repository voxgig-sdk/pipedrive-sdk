...
local RecentEntity = {}
RecentEntity.__index = RecentEntity


function RecentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RecentEntity)
  self._name = "recent"
  self._client = client
...
function RecentEntity:get_name()
  return self._name
end


function RecentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RecentEntity.new(self._client, opts)
end
...
function RecentEntity:mark_deleted()
  self._deleted = true
end


function RecentEntity:deleted()
  return true == self._deleted
end


function RecentEntity:data_set(args)
  if args ~= nil then
...
function RecentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RecentEntity:match_set(args)
  if args ~= nil then
...
function RecentEntity:match_get()
...
function RecentEntity:stream(action, args, callopts)
...
---@param reqmatch RecentListMatch
---@param ctrl? table
---@return Recent[]
---@return string? err
function RecentEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function RecentEntity:_run_op(ctx, post_done)
  local utility = self._utility
