...
local LeadEntity = {}
LeadEntity.__index = LeadEntity


function LeadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LeadEntity)
  self._name = "lead"
  self._client = client
...
function LeadEntity:get_name()
  return self._name
end


function LeadEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LeadEntity.new(self._client, opts)
end
...
function LeadEntity:mark_deleted()
  self._deleted = true
end


function LeadEntity:deleted()
  return true == self._deleted
end


function LeadEntity:data_set(args)
  if args ~= nil then
...
function LeadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LeadEntity:match_set(args)
  if args ~= nil then
...
function LeadEntity:stream(action, args, callopts)
...
---@param reqmatch LeadLoadMatch
---@param ctrl? table
---@return Lead
---@return string? err
function LeadEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch LeadListMatch
---@param ctrl? table
---@return Lead[]
---@return string? err
function LeadEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata LeadCreateData
---@param ctrl? table
---@return Lead
---@return string? err
function LeadEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata LeadUpdateData
---@param ctrl? table
---@return Lead
---@return string? err
function LeadEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch LeadRemoveMatch
---@param ctrl? table
---@return Lead
---@return string? err
function LeadEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function LeadEntity:_run_op(ctx, post_done)
  local utility = self._utility
