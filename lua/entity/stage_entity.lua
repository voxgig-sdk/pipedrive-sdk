...
local StageEntity = {}
StageEntity.__index = StageEntity


function StageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StageEntity)
  self._name = "stage"
  self._client = client
...
function StageEntity:get_name()
  return self._name
end


function StageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StageEntity.new(self._client, opts)
end
...
function StageEntity:mark_deleted()
  self._deleted = true
end


function StageEntity:deleted()
  return true == self._deleted
end


function StageEntity:data_set(args)
  if args ~= nil then
...
function StageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StageEntity:match_set(args)
  if args ~= nil then
...
function StageEntity:match_get()
...
function StageEntity:stream(action, args, callopts)
...
---@param reqmatch StageListMatch
---@param ctrl? table
---@return Stage[]
---@return string? err
function StageEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function StageEntity:_run_op(ctx, post_done)
  local utility = self._utility
