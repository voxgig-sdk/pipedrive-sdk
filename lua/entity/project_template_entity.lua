...
local ProjectTemplateEntity = {}
ProjectTemplateEntity.__index = ProjectTemplateEntity


function ProjectTemplateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectTemplateEntity)
  self._name = "project_template"
  self._client = client
...
function ProjectTemplateEntity:get_name()
  return self._name
end


function ProjectTemplateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectTemplateEntity.new(self._client, opts)
end
...
function ProjectTemplateEntity:mark_deleted()
  self._deleted = true
end


function ProjectTemplateEntity:deleted()
  return true == self._deleted
end


function ProjectTemplateEntity:data_set(args)
  if args ~= nil then
...
function ProjectTemplateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectTemplateEntity:match_set(args)
  if args ~= nil then
...
function ProjectTemplateEntity:match_get()
...
function ProjectTemplateEntity:stream(action, args, callopts)
...
---@param reqmatch ProjectTemplateLoadMatch
---@param ctrl? table
---@return ProjectTemplate
---@return string? err
function ProjectTemplateEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ProjectTemplateListMatch
---@param ctrl? table
---@return ProjectTemplate[]
---@return string? err
function ProjectTemplateEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ProjectTemplateEntity:_run_op(ctx, post_done)
  local utility = self._utility
