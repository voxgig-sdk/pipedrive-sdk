...
local OauthEntity = {}
OauthEntity.__index = OauthEntity


function OauthEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OauthEntity)
  self._name = "oauth"
  self._client = client
...
function OauthEntity:get_name()
  return self._name
end


function OauthEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OauthEntity.new(self._client, opts)
end
...
function OauthEntity:mark_deleted()
  self._deleted = true
end


function OauthEntity:deleted()
  return true == self._deleted
end


function OauthEntity:data_set(args)
  if args ~= nil then
...
function OauthEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OauthEntity:match_set(args)
  if args ~= nil then
...
function OauthEntity:match_get()
...
function OauthEntity:stream(action, args, callopts)
...
function OauthEntity:_run_op(ctx, post_done)
  local utility = self._utility
