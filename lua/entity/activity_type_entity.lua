...
local ActivityTypeEntity = {}
ActivityTypeEntity.__index = ActivityTypeEntity


function ActivityTypeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActivityTypeEntity)
  self._name = "activity_type"
  self._client = client
...
function ActivityTypeEntity:get_name()
  return self._name
end


function ActivityTypeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActivityTypeEntity.new(self._client, opts)
end
...
function ActivityTypeEntity:mark_deleted()
  self._deleted = true
end


function ActivityTypeEntity:deleted()
  return true == self._deleted
end


function ActivityTypeEntity:data_set(args)
  if args ~= nil then
...
function ActivityTypeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActivityTypeEntity:match_set(args)
  if args ~= nil then
...
function ActivityTypeEntity:match_get()
...
function ActivityTypeEntity:stream(action, args, callopts)
...
---@param reqmatch ActivityTypeListMatch
---@param ctrl? table
---@return ActivityType[]
---@return string? err
function ActivityTypeEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata ActivityTypeCreateData
---@param ctrl? table
---@return ActivityType
---@return string? err
function ActivityTypeEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata ActivityTypeUpdateData
---@param ctrl? table
---@return ActivityType
---@return string? err
function ActivityTypeEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch ActivityTypeRemoveMatch
---@param ctrl? table
---@return ActivityType
---@return string? err
function ActivityTypeEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function ActivityTypeEntity:_run_op(ctx, post_done)
  local utility = self._utility
