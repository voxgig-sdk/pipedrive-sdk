...
local MailboxEntity = {}
MailboxEntity.__index = MailboxEntity


function MailboxEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MailboxEntity)
  self._name = "mailbox"
  self._client = client
...
function MailboxEntity:get_name()
  return self._name
end


function MailboxEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MailboxEntity.new(self._client, opts)
end
...
function MailboxEntity:mark_deleted()
  self._deleted = true
end


function MailboxEntity:deleted()
  return true == self._deleted
end


function MailboxEntity:data_set(args)
  if args ~= nil then
...
function MailboxEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MailboxEntity:match_set(args)
  if args ~= nil then
...
function MailboxEntity:match_get()
...
function MailboxEntity:stream(action, args, callopts)
...
---@param reqmatch MailboxLoadMatch
---@param ctrl? table
---@return Mailbox
---@return string? err
function MailboxEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch MailboxListMatch
---@param ctrl? table
---@return Mailbox[]
---@return string? err
function MailboxEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata MailboxUpdateData
---@param ctrl? table
---@return Mailbox
---@return string? err
function MailboxEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch MailboxRemoveMatch
---@param ctrl? table
---@return Mailbox
---@return string? err
function MailboxEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function MailboxEntity:_run_op(ctx, post_done)
  local utility = self._utility
