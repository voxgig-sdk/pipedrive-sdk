...
describe("ProductEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_PRODUCT_ENTID JSON to run live")
      return
    end
...
function product_basic_setup(extra)
  ...
  local idmap = vs.transform(
    { "product01", "product02", "product03", "follower01", "follower02", "follower03" },
    {
      ["`$PACK`"] = { "", {
        ["`$KEY`"] = "`$COPY`",
  ...
  local entid_env_raw = os.getenv("PIPEDRIVE_TEST_PRODUCT_ENTID")
