...
describe("LeadDirect", function()
  it("should direct-list-lead", function()
    local setup = lead_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-lead", setup.live and "live" or "unit")
  ...
  it("should direct-load-lead", function()
    local setup = lead_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-lead", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
