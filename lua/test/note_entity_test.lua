...
describe("NoteEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Note(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Note(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_NOTE_ENTID JSON to run live")
      return
    end
    ...
    -- LIST
    local note_ref01_match = {}

    local note_ref01_list_result, err = note_ref01_ent:list(note_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(note_ref01_list_result)
    ...
    local note_ref01_markdef_up0_name = "add_time"
    ...
    -- REMOVE
    local note_ref01_match_rm0 = {
      id = note_ref01_data["id"],
    }
    local _, err = note_ref01_ent:remove(note_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local note_ref01_match_rt0 = {}

    local note_ref01_list_rt0_result, err = note_ref01_ent:list(note_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(note_ref01_list_rt0_result)
...
function note_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("PIPEDRIVE_TEST_NOTE_ENTID")
