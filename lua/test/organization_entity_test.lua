...
describe("OrganizationEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_ORGANIZATION_ENTID JSON to run live")
      return
    end
...
function organization_basic_setup(extra)
  ...
  local idmap = vs.transform(
    { "organization01", "organization02", "organization03", "follower01", "follower02", "follower03" },
    {
      ["`$PACK`"] = { "", {
        ["`$KEY`"] = "`$COPY`",
  ...
  local entid_env_raw = os.getenv("PIPEDRIVE_TEST_ORGANIZATION_ENTID")
