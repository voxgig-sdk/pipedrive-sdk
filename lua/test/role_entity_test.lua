...
describe("RoleEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_ROLE_ENTID JSON to run live")
      return
    end
    ...
    -- UPDATE
    local role_ref01_data_up0_up = {
      id = role_ref01_data["id"],
    }

    local role_ref01_markdef_up0_name = "name"
    local role_ref01_markdef_up0_value = "Mark01-role_ref01_" .. tostring(setup.now)
    role_ref01_data_up0_up[role_ref01_markdef_up0_name] = role_ref01_markdef_up0_value

    local role_ref01_resdata_up0_result, err = role_ref01_ent:update(role_ref01_data_up0_up, nil)
    assert.is_nil(err)
    local role_ref01_resdata_up0 = helpers.to_map(type(role_ref01_resdata_up0_result) == 'table' and role_ref01_resdata_up0_result.data_get and role_ref01_resdata_up0_result:data_get() or role_ref01_resdata_up0_result)
    assert.is_not_nil(role_ref01_resdata_up0)
    assert.are.equal(role_ref01_resdata_up0["id"], role_ref01_data_up0_up["id"])
    assert.are.equal(role_ref01_resdata_up0[role_ref01_markdef_up0_name], role_ref01_markdef_up0_value)
...
function role_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("PIPEDRIVE_TEST_ROLE_ENTID")
