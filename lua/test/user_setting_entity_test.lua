...
describe("UserSettingEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_USER_SETTING_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local user_setting_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.user_setting")))
    local user_setting_ref01_data = nil
    if #user_setting_ref01_data_raw > 0 then
      user_setting_ref01_data = helpers.to_map(user_setting_ref01_data_raw[1][2])
    end
    ...
    assert.is_nil(err)
    assert.is_not_nil(user_setting_ref01_data_dt0_loaded)
...
function user_setting_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("PIPEDRIVE_TEST_USER_SETTING_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["PIPEDRIVE_TEST_USER_SETTING_ENTID"])
