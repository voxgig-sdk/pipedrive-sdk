...
describe("ProjectEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set PIPEDRIVE_TEST_PROJECT_ENTID JSON to run live")
      return
    end
...
function project_basic_setup(extra)
  ...
  local idmap = vs.transform(
    { "project01", "project02", "project03", "activity01", "activity02", "activity03", "task01", "task02", "task03" },
    {
      ["`$PACK`"] = { "", {
        ["`$KEY`"] = "`$COPY`",
  ...
  local entid_env_raw = os.getenv("PIPEDRIVE_TEST_PROJECT_ENTID")
