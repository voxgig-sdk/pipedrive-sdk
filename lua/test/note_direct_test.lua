...
describe("NoteDirect", function()
  it("should direct-list-note", function()
    local setup = note_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-note", setup.live and "live" or "unit")
  ...
  it("should direct-load-note", function()
    local setup = note_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-note", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
