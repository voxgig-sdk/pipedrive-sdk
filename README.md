...
Pipedrive Deals API client, generated from the OpenAPI spec.

Create, read, update, and delete Pipedrive deals.

Learn more about Pipedrive Deals API at [pipedrive.com](https://pipedrive.com).

This is an unofficial SDK for the Pipedrive Deals public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Deal — that you
...
const items = await client.Deal().list()
```
...
// deals is an array of Deal entities, populated with mock data
// — call deals[0].data() for the record itself
console.log(deals)
```
...
```go
client := sdk.Test()
result, err := client.Deal(nil).List(
    nil, nil,
)
```
...
const client = PipedriveSDK.test()
const deals = await client.Deal().list()
// deals is an array of entities, populated with mock data
// — call deals[0].data() for the record itself
console.log(deals)
```
...
// List all deals (returns DealEntity[] — .data() for the record)
const deals = await client.Deal().list()
...
| --- | --- | --- |
| **Deal** | The Deal entity (create, list, load, remove, update). | `/deals` |
...
# List all deals (returns a list, raises on error)
deals = client.Deal().list()
for deal in deals:
    print(deal)

# Load a specific deal (returns the record, raises on error)
...
// List all deals (returns an array; throws on error)
$deals = $client->Deal()->list();
print_r(array_map(fn($item) => $item->data_get(), $deals));

// Load a specific deal (returns the ENTITY; call data_get() for the record; throws on error)
$deal = $client->Deal()->load(["id" => 1]);
...
// List all deals (returns an array)
const deals = await client.Deal().list()
for (const deal of deals) {
  console.log(deal)
}
...
Generated from the Pipedrive Deals API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
