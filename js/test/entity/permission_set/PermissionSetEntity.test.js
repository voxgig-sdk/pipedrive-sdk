...
describe('PermissionSetEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    let permission_set_ref01_data = Object.values(setup.data.existing.permission_set)[0]

    // LIST
    const permission_set_ref01_ent = client.PermissionSet()
    const permission_set_ref01_match = {}

    const permission_set_ref01_list = (await permission_set_ref01_ent.list(permission_set_ref01_match)).map((e) => e.data())


    // LOAD
    const permission_set_ref01_match_dt0 = {}
