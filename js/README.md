# Pipedrive JavaScript SDK



The JavaScript SDK for the Pipedrive API — an entity-oriented client with full async/await support.
...
### List ActivityField Records

```js
const activity_fields = await client.ActivityField().list()
for (const activity_field of activity_fields) {
...
```js
const client = new PipedriveSDK({ apikey: '...' })
...
| `weighted_values_total` | `Object` | The total weighted values of the deals grouped by deal currency. |
...
| Field | Type | Description |
| --- | --- | --- |
| `conditions` | `Object` | The conditions of the filter as a JSON object. |
| `data` | `Object` | The filter object including conditions |
| `id` | `string` |  |
...
| Field | Type | Description |
| --- | --- | --- |
| `assignee` | `Object` | Who this goal is assigned to. |
...
| `value` | `Object` | The potential value of the lead represented by a JSON object: `{ "amount": 200, "currency": "EUR" }`. |
...
pipedrive/
├── src/
│   ├── PipedriveSDK.js        # Main SDK class
