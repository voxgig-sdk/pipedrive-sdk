...
`client.Deal()` — each with a small set of operations (`list`, `load`, `create`, `update`, `remove`)
...
```js
const deal = await client.Deal().load({ id: 1 })
console.log(deal)
```
...
const deal = await client.Deal().list()
// deal is the entity, populated with mock response data
// — call deal.data() for the record itself
console.log(deal)
```
