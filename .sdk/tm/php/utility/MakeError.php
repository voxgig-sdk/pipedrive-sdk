...
class PipedriveMakeError
{
    public static function call(?PipedriveContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new PipedriveContext([], null);
        }
        $op = $ctx->op ?? new PipedriveOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new PipedriveResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof PipedriveError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new PipedriveError('', $msg, $ctx);
