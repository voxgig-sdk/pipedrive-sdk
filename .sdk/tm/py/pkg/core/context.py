...
from projectname_sdk.core.control import PipedriveControl
...
class PipedriveContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, PipedriveSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, PipedriveResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, PipedriveResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return PipedriveError(code, msg, self)
